...
import { columns } from '@/features/actions/components/columns';
import { PlusIcon } from 'lucide-react'
import  { useEffect, useState } from 'react'
import { useNavigate, useSearchParams } from 'react-router-dom';

import ActionTable from '@/features/actions/components/action-table';
import { useActionService } from '@/features/actions/service/actionService';
import ActionFilterModal from '@/features/actions/components/action-filter-modal';
...
const Actions = () => {
  const navigate = useNavigate();    
    const { getAllAction } = useActionService();
    ...
    const fetchUsers = async (params: Record<string, string | number | boolean>) => {
      try {
        const { content, pageContext } = await getAllAction(params);
  ...
  return (
    ...
    <div className="mb-2 flex flex-wrap items-center justify-between gap-x-4 space-y-2">
      <div>
        ...
        Let your AI Agent interact with other systems. Actions may include: getting customer info, order lookup, submitting tickets, etc.
        </p>
      </div>
    ...
    <ActionTable
      data={data}
      columns={columns({onRowClick: handleEditUser})}
